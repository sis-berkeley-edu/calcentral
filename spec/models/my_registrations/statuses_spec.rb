describe MyRegistrations::Statuses do
  ...
  before(:each) do
    ...
    allow(model).to receive(:student_attributes).and_return(hub_student_attributes[:feed]['student']['studentAttributes'])
    allow(model).to receive(:terms).and_return (stubbed_terms)
  end
  ...
  context 'fully populated berkeley terms' do
    ...
    it 'adds the appropriate term flags' do
      current = model.send(:set_term_flags, stubbed_terms[:current])
      running = model.send(:set_term_flags, stubbed_terms[:running])
