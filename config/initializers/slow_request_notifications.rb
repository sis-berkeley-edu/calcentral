if Settings.logger.slow_request_threshold_in_ms.default.to_i > 0
  ...
  ActiveSupport::Notifications.subscribe "process_action.action_controller" do |name, start, finish, id, payload|
    ...
    payload_path = payload[:path].split('?').first
    threshold = Settings.logger.slow_request_threshold_in_ms.send(payload_path) || Settings.logger.slow_request_threshold_in_ms.default
