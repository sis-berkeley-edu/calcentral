describe MyAcademics::MyHolds do
  describe '#get_feed_internal' do
    ...
    context 'when AcademicStatus response is nil' do
      before do
        allow_any_instance_of(HubEdos::StudentApi::V1::AcademicStatus).to receive(:get).and_return(nil)
      end
      it_behaves_like 'a model receiving no data'
    end
    context 'when AcademicStatus response is empty' do
      before do
        allow_any_instance_of(HubEdos::StudentApi::V1::AcademicStatus).to receive(:get).and_return({})
      end
      it_behaves_like 'a model receiving no data'
    end
    context 'when AcademicStatus returns an error response' do
      before do
        allow_any_instance_of(HubEdos::StudentApi::V1::AcademicStatus).to receive(:get).and_return(failure_response)
      end
    ...
    context 'when AcademicStatus response is populated' do
      before do
        fake_proxy = HubEdos::StudentApi::V1::AcademicStatus.new(fake: true, user_id: '61889')
        allow(HubEdos::StudentApi::V1::AcademicStatus).to receive(:new).and_return fake_proxy
      end
