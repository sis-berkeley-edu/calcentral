...
describe HubEdos::UserAttributes do
  ...
  let(:fake_contact_proxy) { HubEdos::StudentApi::V1::Contacts.new(fake: true, user_id: user_id) }
  before { allow(HubEdos::StudentApi::V1::Contacts).to receive(:new).and_return fake_contact_proxy }
  ...
  describe 'student_id depends on role' do
    before do
      allow_any_instance_of(HubEdos::StudentApi::V1::Contacts).to receive(:get).and_return(
        {feed: {'student' => {
  ...
  context 'Contacts feed is unexpectedly reticent' do
    let(:fake_affiliations_proxy) { HubEdos::StudentApi::V1::Affiliations.new(fake: true, user_id: user_id) }
    before do
      allow_any_instance_of(HubEdos::StudentApi::V1::Contacts).to receive(:get).and_return(
        {feed: {'student' => {}}}
      )
    ...
    it 'reluctantly falls back on the Affiliations API' do
      expect(HubEdos::StudentApi::V1::Affiliations).to receive(:new).and_return fake_affiliations_proxy
      expect(subject[:roles]).to include({applicant: true, releasedAdmit: true})
    end
  end
  ...
  context 'unexpected errors from Hub calls' do
    before do
      allow_any_instance_of(HubEdos::StudentApi::V1::Contacts).to receive(:get_internal).and_return({'non' => 'sense'})
    end
