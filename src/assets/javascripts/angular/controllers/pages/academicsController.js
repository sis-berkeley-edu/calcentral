...
angular
  ...
  .controller('AcademicsController', function(
    ...
    $ngRedux.subscribe(() => {
      const {
        myStatusAndHolds: { termRegistrations = [] },
        myEnrollments: { loaded: enrollmentsLoaded = false },
      } = $ngRedux.getState();

      const shownRegistrations = termRegistrations.find(reg => reg.isShown);
      $scope.showStatusAndBlocks =
        !$scope.filteredForDelegate &&
        ($scope.hasRegStatus ||
          $scope.numberOfHolds ||
          $scope.hasStandingAlert ||
          shownRegistrations);
      $scope.enrollmentsLoaded = enrollmentsLoaded;
    });
    ...
    var parseAcademics = function(response) {
      ...
      $scope.isUndergraduate = _.includes(
        _.get($scope.collegeAndLevel, 'careers'),
        'Undergraduate'
      );
    ...
    var filterWidgets = function() {
      var isNonDegreeSeekingSummerVisitor = academicsService.isNonDegreeSeekingSummerVisitor(
        apiService.user.profile.academicRoles
      );

      $scope.isAcademicInfoAvailable = !!(
        $scope.hasRegStatus ||
      ...
      $scope.showAdvising =
        !$scope.filteredForDelegate &&
        apiService.user.profile.features.advising &&
        apiService.user.profile.roles.student &&
        isMbaJdOrNotLaw() &&
        !isNonDegreeSeekingSummerVisitor;
      $scope.showProfileMessage =
        !$scope.isAcademicInfoAvailable ||
        !$scope.collegeAndLevel ||
        _.isEmpty($scope.collegeAndLevel.careers);
      $scope.showResidency =
        apiService.user.profile.roles.student &&
        academicsService.showResidency(
          apiService.user.profile.academicRoles.current
        );
    ...
    var isMbaJdOrNotLaw = function() {
      return (
        ...
        apiService.user.profile.academicRoles.current.haasMbaJurisDoctor
      );
    };
    ...
    $scope.$on('calcentral.api.user.isAuthenticated', function(
      ...
      if (isAuthenticated) {
        ...
        var getRegistrations = registrationsFactory
          .getRegistrations()
          .then(loadRegistrations);

        const requests = [getAcademics, getRegistrations];

        const { roles, academicRoles } = apiService.user.profile;
        ...
        const loadStandings =
          academicRoles.current.ugrd && !academicRoles.current.ugrdNonDegree;
        ...
        // Load enrollment data if looking at a specific class, required to
        // appropriately filter the Class Information Card callout notice.
        if ($routeParams.classId) {
          requests.push(loadEnrollments());
        }
