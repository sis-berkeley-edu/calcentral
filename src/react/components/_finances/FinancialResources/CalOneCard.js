...
const CalOneCard = ({ links, getLink, expanded }) => {
  ...
  return (
    <HasAccessTo linkNames={ ['debitAccount', 'mealPlanBalance', 'mealPlanLearn'] } links={links}>
      <div className='FinancialResources__categoryContainer' onClick={() => setIsExpanded(!isExpanded)}>
        <FinancialResourcesCategoryHeader title="Cal 1 Card" expanded={ isExpanded } />
        { isExpanded && 
        <ul className="FinancialResources">
          <HasAccessTo linkNames={ ['debitAccount'] } links={links}> 
            { links && <li><APILink { ...getLink('debitAccount', links)} /></li> }
          </HasAccessTo>
          <HasAccessTo linkNames={ ['mealPlanBalance'] } links={links}> 
            { links && <li><APILink { ...getLink('mealPlanBalance', links)} /></li> }
          </HasAccessTo>
          <HasAccessTo linkNames={ ['mealPlanLearn'] } links={links}> 
            { links && <li><APILink { ...getLink('mealPlanLearn', links)} /></li> }
          </HasAccessTo>
        </ul>
        }
      </div>
