describe User::Oauth2Data do
  ...
  it 'should not store plaintext access tokens' do
    ...
    oauth2 = User::Oauth2Data.new(uid: user_id, access_token: 'plaintext')
    expect(oauth2.save).to be true
    access_token = User::Oauth2Data.get(user_id)[:access_token]
    expect(access_token).to_not eq 'plaintext'
  end

  it 'should return decrypted access tokens' do
    oauth2 = User::Oauth2Data.new(uid: user_id, access_token: access_token)
  ...
  it 'should be able to update existing tokens' do
    User::Oauth2Data.new_or_update(
      ...
      refresh_token,
      1)
    tokens = User::Oauth2Data.get(user_id)
    ...
    User::Oauth2Data.new_or_update(user_id, updated_access_token)
    updated_tokens = User::Oauth2Data.get(user_id)
  ...
  it 'should invalidate cache when tokens are deleted' do
    ...
    expect(oauth2.save).to be true
    User::Oauth2Data.where(uid: user_id).destroy_all
    access_token = User::Oauth2Data.get(user_id)[:access_token]
  ...
  it 'should remove dismiss_reminder app_data when a new google token is stored' do
    ...
    User::Oauth2Data.new_or_update(user_id, access_token, refresh_token)
  ...
  it 'new_or_update should merge new app_data into existing app_data' do
    User::Oauth2Data.new_or_update(user_id, access_token, refresh_token, 0, {app_data:{foo: 'foo?'}})
    expect(User::Oauth2Data.send(:get_appdata_field, user_id, :foo)).to eq 'foo?'
    User::Oauth2Data.new_or_update(user_id, access_token, refresh_token, 0, {app_data:{baz: 'baz!'}})
    expect(User::Oauth2Data.send(:get_appdata_field, user_id, :baz)).to eq 'baz!'
    expect(User::Oauth2Data.send(:get_appdata_field, user_id, :foo)).to eq 'foo?'
  end
end
